...
(function(factory) {
	...
	monthYearFormat: 'MMMM YYYY', // required for en. other localeuages rely on datepicker computable option
...
// TODO: rename everything "locale" to "locale", like what the moment project did


// Initialize jQuery UI datepicker translations while using some of the translations
// Will set this as the default localeuage for datepicker.
fc.datepickerLang = function(localeCode, dpLangCode, dpOptions) {

	// get the FullCalendar internal option hash for this localeuage. create if necessary
	...
	if ($.datepicker) {
		...
		// Register the localeuage data.
		// FullCalendar and MomentJS use localeuage codes like "pt-br" but Datepicker
		// does it like "pt-BR" or if it doesn't have the localeuage, maybe just "pt".
		// Make an alias so the localeuage can be referenced either way.
		$.datepicker.regional[dpLangCode] =
			$.datepicker.regional[localeCode] = // alias
				dpOptions;

		// Alias 'en' to the default localeuage data. Do this every time.
...
// Sets FullCalendar-specific translations. Will set the localeuage as the global default.
fc.locale = function(localeCode, newFcOptions) {
	...
	// provided new options for this localeuage? merge them in
	if (newFcOptions) {
		mergeOptions(fcOptions, newFcOptions);
	}

	// compute localeuage options that weren't defined.
	...
	// set it as the default localeuage for FullCalendar
	defaults.locale = localeCode;
};


// NOTE: can't guarantee any of these computations will run because not every localeuage has datepicker
...
function getMomentLocaleData(localeCode) {
	var func = moment.localeData || moment.localeData;
	return func.call(moment, localeCode) ||
...
var dayIDs = [ 'Di', 'Lu', 'Ma', 'Me', 'Je', 'Ve', 'Sa' ];
...
function formatRange(date1, date2, formatStr, separator, isRTL) {
	...
	localeData = (date1.localeData || date1.locale).call(date1); // works with moment-pre-2.8
...
var TimeGrid = Grid.extend({
	...
	computeColHeadFormat: function() {
		if (this.colCnt > 1) { // multiple days, so full single date string WON'T be in title text
			return 'dddd';this.view.opt('dayOfMonthFormat'); // "Sat 12/10"
		}
		else { // single day, so full single date string will probably be in title text
			return 'dddd'; // "Saturday"
		}
...
var View = fc.View = Class.extend({
	...
	// -----------------------------------------------------------------------------------
	// Precedence (lowest to highest): defaults, rtlDefaults, localeOptions, instanceOptions
	...
	// determine localeuage options
	if (options.locale in localeOptionHash) {
		localeOptions = localeOptionHash[options.locale];
	}
	else {
		localeOptions = localeOptionHash[defaults.locale];
	}

	if (localeOptions) { // if localeuage options exist, rebuild...
		options = mergeOptions({}, defaults, localeOptions, instanceOptions);
	}

	if (options.isRTL) { // is isRTL, rebuild...
		options = mergeOptions({}, defaults, rtlDefaults, localeOptions || {}, instanceOptions);
	}
	...
	// -----------------------------------------------------------------------------------
	// Apply overrides to the current localeuage's data


	var localeData = createObject( // make a cheap copy
		getMomentLocaleData(options.locale) // will fall back to en
	);
	...
	// Builds a moment using the settings of the current calendar: timezone and localeuage.
	...
	function humanizeDuration(duration) {
		return (duration.locale || duration.locale).call(duration, options.locale) // works moment-pre-2.8
			.humanize();
	}
	...
	function getViewSpec(requestedViewType) {
		...
		if (viewClass) {
			...
			buttonText =
				...
				allDefaultButtonText[requestedViewType] || // locale data, like "agendaWeek"
				(unitIsSingle ? allDefaultButtonText[unit] : null) || // locale data, like "week"
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
						...
						if (buttonClick) {
							...
							defaultText = smartProperty(options.defaultButtonText, buttonName); // from localeuages
